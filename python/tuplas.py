...
# Se defien una lista de tuplas donde cada tupla contiene (ID, Nombre, Edad)
registros = [
    (1, "Ana", 25),
    (2, "Luis", 30),
    (3, "Carlos", 22)
...
def consultar(nombre):
    """
    Busca registros en la lista cuya persona tenga el nombre dado (sin distinguir mayusculas/minusculas).
...
# Funcion para ordenar las lista por clave 
def ordenar_por_clave(indice):
    """
    Ordena la lista de registros segun el indice de la clave especificada (0: ID, 1: Nombre, 2: Edad).
    :param indice: Indice de la clave por la cual ordenar.
...
# Funcion para almacenar en otra lista los registros mayores a una edad
def filtrar_por_edad(edad_minima):
    ...
    :return: Lista de tuplas con losd registros filtrados
    """
...
def menu():
    while True:
        ...
        if opcion == "1":
            ...
            clave = input("Ingrese la calve de ordenamiento (id, nombre, edad): ").lower()
